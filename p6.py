...
def get_rate_from_provider(provider_label):
    if provider_label == "Meralco":
        return 13.4702
    elif provider_label == "BATELEC I":
        return 9.7297
    else:
        return 8.7447

def compute_energy(power, hours, provider_label):
    """
    power: Watts
    hours: hours for that specific date (float)
    provider_label: "Meralco" / "BATELEC I" / "BATELEC II"
    Returns energy_kwh (for that date) and cost (for that date)
    """
    rate = get_rate_from_provider(provider_label)
    energy_kwh = (power * hours) / 1000.0
    cost = energy_kwh * rate
    return energy_kwh, cost

def update_stats_for_dashboard(appliances_list, selected_date_iso):
    """
    appliances_list: list of saved entries for the selected month (each entry includes "date", "energy_kwh", "cost", "name")
    selected_date_iso: iso string of the selected_date (YYYY-MM-DD)
    Returns: total_unique_appliances, energy_on_selected_date, cost_on_selected_date, monthly_energy_total, monthly_cost_total
    """
    if not appliances_list:
        return 0, 0.0, 0.0, 0.0, 0.0

    df = pd.DataFrame(appliances_list)

    total_unique_appliances = df["name"].nunique()

    try:
        selected_day_df = df[df["date"] == selected_date_iso]
    except Exception:
        selected_day_df = pd.DataFrame()

    energy_selected_day = selected_day_df["energy_kwh"].sum() if not selected_day_df.empty else 0.0
    cost_selected_day = selected_day_df["cost"].sum() if not selected_day_df.empty else 0.0

    monthly_energy_total = df["energy_kwh"].sum()
    monthly_cost_total = df["cost"].sum()

    return total_unique_appliances, energy_selected_day, cost_selected_day, monthly_energy_total, monthly_cost_total
...
def generate_green_bill(month_key, top_appliance, provider, monthly_energy, monthly_cost, breakdown):
    ...
    path = f"Energy_Saver_Bill_{month_safe}.pdf"
...
if st.session_state.page == "Dashboard":
    ...
    if "_sync_calendar" in st.session_state:
        st.session_state.dashboard_month = st.session_state._sync_calendar["month"]
        st.session_state.dashboard_year = st.session_state._sync_calendar["year"]
        del st.session_state._sync_calendar
    ...
    with col_ms1:
        selected_month = st.selectbox(
            "Select Month",
            months_list,
            index=months_list.index(st.session_state.dashboard_month)
        )
    ...
    with col_ms2:
        selected_year = st.number_input(
            ...
            min_value=2000,
            max_value=2100,
            value=st.session_state.dashboard_year,
            step=1
        )

    if selected_month != st.session_state.dashboard_month:
        st.session_state.dashboard_month = selected_month

    if selected_year != st.session_state.dashboard_year:
        st.session_state.dashboard_year = selected_year

    st.session_state.current_viewing_month = st.session_state.dashboard_month
    st.session_state.current_viewing_year = st.session_state.dashboard_year

    selected_month_index = months_list.index(st.session_state.dashboard_month) + 1
    auto_synced_date = datetime(
        st.session_state.dashboard_year,
        selected_month_index,
        1
    )

    prev_month = st.session_state.get("_prev_dashboard_month")
    prev_year = st.session_state.get("_prev_dashboard_year")

    if (prev_month != st.session_state.dashboard_month or
        prev_year != st.session_state.dashboard_year):
        st.session_state.usage_date = auto_synced_date

    st.session_state._prev_dashboard_month = st.session_state.dashboard_month
    st.session_state._prev_dashboard_year = st.session_state.dashboard_year

    selected_date = st.date_input(
        "Select Date of Usage",
        st.session_state.get("usage_date", auto_synced_date),
        key="usage_date"
    )
    ...
    st.session_state.appliances = list(st.session_state.appliances_by_month[month_key])

    st.markdown("---")

    # ---------------- Dashboard Statistics ----------------
    if 'usage_date' in st.session_state and st.session_state.usage_date:
        selected_date_iso = st.session_state.usage_date.isoformat()
        display_date = st.session_state.usage_date
    else:
        selected_date_iso = datetime.now().date().isoformat()
        display_date = datetime.now().date()
    
    total_unique_appliances, energy_selected_day, cost_selected_day, monthly_energy_total, monthly_cost_total = update_stats_for_dashboard(
        st.session_state.appliances, selected_date_iso
    )

    col1, col2, col3, col4 = st.columns(4)
    
    cards = [
        ("#1C9E81", "🧰", "Total Appliances", f"{total_unique_appliances}"),
        ("#0096E6", "⚡", f"Daily Energy ({display_date.strftime('%b %d')})", f"{energy_selected_day:.2f} kWh"),
        ("#F79C1D", "💰", f"Daily Cost ({display_date.strftime('%b %d')})", f"₱{cost_selected_day:.2f}"),
        ("#9336B4", "📅", "Monthly Total", f"₱{monthly_cost_total:.2f}")
    ]
    ...
    with st.form(key=f"add_appliance_form_{st.session_state.form_counter}", clear_on_submit=True):
        ...
        if compute_btn:
            if not name or not category or not provider:
                st.error("⚠️ Please fill out all fields first.")
            else:
                provider_label = (
                    "Meralco" if "Meralco" in provider else
                    "BATELEC I" if "BATELEC I" in provider else
                    "BATELEC II"
                )

                st.session_state.temp_values = {
                    'name': name,
                    'category_idx': ["", "Kitchen", "Entertainment", "Lighting", "Cooling & Heating", "Laundry", "Other"].index(category),
                    'power': power,
                    'hours': hours,
                    'provider_idx': ["", "Meralco (₱13.4702/kWh)", "BATELEC I (₱9.7297/kWh)", "BATELEC II (₱8.7447/kWh)"].index(provider)
                }

                energy_kwh, cost = compute_energy(power, hours, provider_label)
    ...
    if 'pending_appliance' in st.session_state and st.session_state.pending_appliance:
        ...
        with preview_col1:
            ...
            <div style="background-color: #f0f2f6; padding: 15px; border-radius: 10px; border-left: 4px solid #0096E6;">
                ...
                <p style="margin: 5px 0;"><strong>Date:</strong> {pending['date']}</p>
            </div>
            """, unsafe_allow_html=True)
...
# -------------------- Consumption Summary --------------------
    ...
    if "current_viewing_month" not in st.session_state:
        st.session_state.current_viewing_month = datetime.now().strftime("%B")
    if "current_viewing_year" not in st.session_state:
        st.session_state.current_viewing_year = datetime.now().year
    ...
    with col1:
        selected_month = st.selectbox(
            "Select Month",
            months_list,
            index=months_list.index(st.session_state.current_viewing_month),
            key="summary_month_select"
        )
    with col2:
        selected_year = st.number_input(
            "Year",
            min_value=2000,
            max_value=2100,
            value=st.session_state.current_viewing_year,
            step=1,
            key="summary_year_select"
        )

    st.session_state.current_viewing_month = selected_month
    st.session_state.current_viewing_year = selected_year
    month_key = f"{selected_month} {selected_year}"
    st.caption(f"Showing data for: **{month_key}**")

    appliances = st.session_state.appliances_by_month.get(month_key, [])
    ...
    if appliances:
        ...
        if "date" in df.columns:
            ...
            daily_energy = daily_groups["energy_kwh"].mean()
            daily_cost = daily_groups["cost"].mean()
            daily_groups['date_formatted'] = pd.to_datetime(daily_groups['date']).dt.strftime('%b %d, %Y')
        else:
            daily_groups = pd.DataFrame(columns=["date", "energy_kwh", "cost"])
            daily_energy = daily_cost = 0.0

        monthly_energy = df["energy_kwh"].sum()
        monthly_cost = df["cost"].sum()

        yearly_energy = yearly_cost = 0.0
        for key, entries in st.session_state.appliances_by_month.items():
            if str(selected_year) in key and entries:
                df_year = pd.DataFrame(entries)
                yearly_energy += df_year["energy_kwh"].sum()
                yearly_cost += df_year["cost"].sum()

        summary_html = f"""
        <div style="display:flex; flex-wrap:wrap; gap:15px; justify-content:space-between; margin-bottom:20px;">
            <div style="flex:1; min-width:150px; background:#E8F5E9; padding:20px; border-radius:12px; text-align:center;">
                <h4>Total Daily</h4>
                <h3 style="color:#2E7D32;">{daily_energy:.2f} kWh</h3>
                <p>₱{daily_cost:.2f}</p>
            </div>
            <div style="flex:1; min-width:150px; background:#E3F2FD; padding:20px; border-radius:12px; text-align:center;">
                <h4>Monthly Total</h4>
                <h3 style="color:#1976D2;">{monthly_energy:.2f} kWh</h3>
                <p>₱{monthly_cost:.2f}</p>
            </div>
            <div style="flex:1; min-width:150px; background:#FFF3E0; padding:20px; border-radius:12px; text-align:center;">
                <h4>Yearly Total</h4>
                <h3 style="color:#FB8C00;">{yearly_energy:.2f} kWh</h3>
                <p>₱{yearly_cost:.2f}</p>
            </div>   
        </div> 
        """
        ...
        if not daily_groups.empty:
            st.markdown("<h4>📅 Daily Breakdown</h4>", unsafe_allow_html=True)
            daily_html = ""
            for _, row in daily_groups.iterrows():
                ...
                <div style="background-color:#E8F5E9; padding:10px; border-radius:8px; margin:5px 0;">
                    ...
                    Energy: {row['energy_kwh']:.2f} kWh | Cost: ₱{row['cost']:.2f}
                </div>
                """
        ...
        if st.session_state.get("chart_type"):
            if st.session_state.chart_type == "Bar Chart":
                fig = px.bar(df, x="name", y="energy_kwh", text_auto='.2f',
                             title=f"Energy Consumption by Appliance - {month_key} (kWh)")
                fig.update_layout(xaxis_title="Appliance", yaxis_title="Energy (kWh)", xaxis_tickangle=30, height=450)
                st.plotly_chart(fig, use_container_width=True)
            else:
                fig = px.pie(df, names="name", values="cost",
                             title=f"Cost Distribution by Appliance - {month_key} (₱)")
                fig.update_traces(textinfo="label+value", textposition="inside")
                st.plotly_chart(fig, use_container_width=True)

    else:
        st.info(f"No appliances logged yet for **{month_key}**.")
...
# -------------------- Monthly Report --------------------
    ...
    if appliances:
        ...
        if generate:
            provider = df["provider"].mode()[0] if not df["provider"].empty else "Meralco"
        ...
        cards = [
            ("#1C9E81", "🧰", "Appliances", df["name"].nunique()),
            ("#0096E6", "⚡", "Energy", f"{monthly_energy:.2f} kWh"),
            ("#F79C1D", "💰", "Cost", f"₱{monthly_cost:.2f}"),
            ("#9336B4", "🏷️", "Provider", df["provider"].mode()[0] if not df["provider"].empty else "N/A")
        ]
...
"""
        ...
        st.info("No appliances to display for this month. Add appliance usages on the Dashboard.")
...
# -------------------- CARBON FOOTPRINT --------------------
    ...
    if st.session_state.get("calc_carbon", False):
        ...
        if appliances:
            ...
            daily_energy = df.groupby("date")["energy_kwh"].sum().mean()
            monthly_energy = df["energy_kwh"].sum()
            ...
            labels = ["Daily (avg)", "Monthly", "Yearly"]
            ...
            fig = px.bar(
                ...
                y=values,
                text_auto='.2f',
                title="CO₂ Emissions (Daily avg vs Monthly vs Yearly)"
            )
            fig.update_layout(
            ...
            offset_actions = {
                ...
                "Use LED bulbs instead of incandescent": 0.05,
            ...
            if selected_action == "Plant a tree":
                ...
                co2_saved = standby_power_per_device * num_devices * CO2_PER_KWH
                ...
                avg_bulb_kw = 0.01
                kwh_saved_per_day = avg_bulb_kw * hours_saved * num_bulbs
                co2_saved = kwh_saved_per_day * CO2_PER_KWH

            else:
                count = st.slider("How many times will you apply this action?", 1, 10, 1)
                co2_saved = (offset_actions[selected_action] or 0.0) * count
            ...
            st.info("No appliances found for the selected month. Add appliances first to calculate CO₂.")

# ===================== Work & Energy Calculator =====================
elif st.session_state.page == "Work & Energy Analysis":
    st.subheader("🧮 Work & Energy Analysis")
